...
#Example of using private docker registry:
scripts/loadImages.sh -p ida-25.0.2.tgz -r $REGISTRY_HOST/ida
```
...
scripts/deployOperator.sh -i $REGISTRY_HOST/ida/ida-operator:25.0.2 -s ida-operator-secret

#Example of cluster-scoped operator:
scripts/deployOperator.sh -i $REGISTRY_HOST/ida/ida-operator:25.0.2 -c Cluster -s ida-operator-secret
...
```
#Example of using private docker registry:
oc set image deployment/ida-operator operator=$REGISTRY_HOST/ida/ida-operator:25.0.2
```
...
- **Configuring IDA Custom Resource**
  ...
  shared.imageTag | Image tag for IDA and Operator, can be overridden individually. E.g., 25.0.2
...
```
kubectl patch --type=merge idacluster/idadeploy -p '{"spec": {"shared": {"imageTag": "25.0.2"}}}'
```
