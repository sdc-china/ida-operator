...
#Example of using private docker registry:
scripts/loadImages.sh -p ida-25.0.3-java17.tgz -r $REGISTRY_HOST/ida
```
...
scripts/deployOperator.sh -i $REGISTRY_HOST/ida/ida-operator:25.0.3 -s ida-operator-secret

#Example of cluster-scoped operator:
scripts/deployOperator.sh -i $REGISTRY_HOST/ida/ida-operator:25.0.3 -c Cluster -s ida-operator-secret
...
- **Configuring IDA Custom Resource**
  ...
  shared.imageTag | Image tag for IDA and Operator, can be overridden individually. E.g., 25.0.3
...
```
kubectl patch --type=merge idacluster/idadeploy -p '{"spec": {"shared": {"imageTag": "25.0.3"}}}'
```
