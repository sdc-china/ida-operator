...
#Example of using private docker registry:
scripts/loadImages.sh -p ida-25.0.8-1-java17.tgz -r $REGISTRY_HOST/ida
```
...
#Example of operator:
scripts/deployOperator.sh -i $REGISTRY_HOST/ida/ida-operator:25.0.8-1 -s ida-operator-secret
...
```
#Example of using private docker registry:
oc set image deployment/ida-operator operator=$REGISTRY_HOST/ida/ida-operator:25.0.8-1
```
...
- **Configuring IDA Custom Resource**
  ...
  shared.imageTag | Image tag for IDA and Operator, can be overridden individually. E.g., 25.0.8-1
...
```
kubectl patch --type=merge idacluster/idadeploy -p '{"spec": {"shared": {"imageTag": "25.0.8-1"}}}'
```
