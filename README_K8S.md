...
#Example of using private docker registry:
scripts/loadImages.sh -p ida-24.0.10-2.1.tgz -r $REGISTRY_HOST/ida
```
...
scripts/deployOperator.sh -i $REGISTRY_HOST/ida/ida-operator:24.0.10-2.1 -s ida-operator-secret

#Example of cluster-scoped operator:
scripts/deployOperator.sh -i $REGISTRY_HOST/ida/ida-operator:24.0.10-2.1 -c Cluster -s ida-operator-secret
...
```
#Example of using private docker registry:
oc set image deployment/ida-operator operator=$REGISTRY_HOST/ida/ida-operator:24.0.10-2.1
```
...
- **Configuring IDA Custom Resource**
  ...
  shared.imageTag | Image tag for IDA and Operator, can be overridden individually. E.g., 24.0.10-2.1
...
```
kubectl patch --type=merge idacluster/idadeploy -p '{"spec": {"shared": {"imageTag": "24.0.10-2.1"}}}'
```
