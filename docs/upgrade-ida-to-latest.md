...
scripts/loadImages.sh -p ida-26.0.4-java17.tar -r $REGISTRY_HOST

#Example of loading tgz file and using private docker registry:
scripts/loadImages.sh -p ida-26.0.4-java17.tgz -r $REGISTRY_HOST
```
...
Step 2. Updrade IDA operator to v26.0.4.

```
oc get deployment | grep ida-operator | awk '{print $1}' | xargs oc rollout pause deployment

oc set image deployment/ida-operator operator=$REGISTRY_HOST/ida-operator:26.0.4
```
...
Step 2. Updrade IDA to v26.0.4.

```
oc get deployment | grep ida-web | awk '{print $1}' | xargs oc rollout pause deployment

oc patch --type=merge idacluster/idadeploy -p '{"spec": {"shared": {"imageTag": "26.0.4"}}}'
